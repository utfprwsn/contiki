...
uint8_t buffer_pool[(MQTT_SN_MAX_PACKET_LENGTH+BUFFER_OFFSET)*MAX_TOPIC_USED];
...
void init_buffers()
{
    ...
    {
        g_topic_bind[i].topic_name = (char*)buffer_pool+i*(MQTT_SN_MAX_PACKET_LENGTH+BUFFER_OFFSET);
    }
}
...
void parse_mqtt_type_string(uint8_t type, char **type_string){
  switch (type) {
    ...
    break;
    default:
      *type_string = "Unespecified state";
    break;
  }
}
...
resp_con_t mqtt_sn_sub(char *topic, uint8_t qos){
  ...
  if(verf_hist_sub(topic)){
    ...
    if (!mqtt_sn_insert_queue(subscribe_task))
     debug_task("Error while enqueuing sub task");
...
void mqtt_sn_recv_parser(const uint8_t *data){
    ...
    switch (msg_type) {
      ...
      case MQTT_SN_TYPE_REGISTER:
        ...
        char* buff; // = g_topic_bind[j].topic_name;
        short_topic = data[3];
        ...
        buff = g_topic_bind[j].topic_name;
